...
- The dynamical law of quantum mechanics — guessed, not derived, and none the worse for it.
- The plausibility argument, the Born rule and what $|\psi|^2$ means, why Hermiticity keeps probability conserved, stationary states, and the rest.
...
In late 1925, Erwin Schrödinger took a Christmas holiday in the Swiss Alps, at the Villa Herwig in Arosa, and came back down in January 1926 with an equation. Walter Moore's biography (*Schrödinger: Life and Thought*, Cambridge University Press, 1989) [*verify exact chapter*] covers the trip; the physics folklore supplies the more colorful details about who went along. What is not in doubt is the result. Four papers appeared in *Annalen der Physik* starting in January 1926 [Schrödinger, *Ann. Phys.* 79, 361–376](https://doi.org/10.1002/andp.19263840404), and within months they had reproduced every known energy level of hydrogen, explained the selection rules for atomic transitions, and handed physicists a formalism they could actually compute with.
<!-- FACT-CHECK FLAG: UNVERIFIED — see factchecks/03-the-schrodinger-equation-assertions.md -->

Heisenberg, Born, and Jordan had reached the same physics six months earlier through matrix mechanics. But matrix mechanics looked like nothing classical physicists had ever seen — arrays of numbers obeying unfamiliar multiplication rules. Schrödinger's equation looked like a wave equation, and physicists are at home with wave equations. The wave-equation formulation became the standard entry point almost overnight, and it has stayed that way.

Here is where we stand. Unit 1 supplied the experimental evidence that forced quantum mechanics into being: Planck's blackbody radiation, the photoelectric effect, Compton scattering, de Broglie's matter waves, Franck–Hertz. Unit 2 supplied the mathematical apparatus: complex Hilbert spaces, inner products, Hermitian operators, eigenvalue equations. What is missing is a *dynamical law* — a rule for how a quantum state changes in time. Classical mechanics has Newton's $\vec{F} = m\vec{a}$; quantum mechanics needs its counterpart. The Schrödinger equation is it.

But — and this is the move the chapter has to make honestly — *the Schrödinger equation cannot be derived.* Griffiths says so flatly in §1.1 of the third edition: "Where did Schrödinger's equation come from? In a sense, it cannot be derived." There is a plausibility argument that makes the equation look natural given what de Broglie and Einstein already told us, and the argument is worth running. But running it is not deriving the equation. The Schrödinger equation has exactly the status of Newton's second law: a fundamental postulate, accepted for what it explains, not because some deeper theory implies it. Asking why nature uses this equation rather than a neighboring one is a question with no deeper answer inside non-relativistic quantum mechanics.
<!-- FACT-CHECK FLAG: UNVERIFIED — see factchecks/03-the-schrodinger-equation-assertions.md -->
...
Start with de Broglie's matter wave for a free particle moving in the $+x$ direction:
...
For a free particle, $E = p^2/2m$. So the two sides of the last two equations must agree, and you get the free-particle equation:

$$i\hbar\frac{\partial\psi}{\partial t} = -\frac{\hbar^2}{2m}\frac{\partial^2\psi}{\partial x^2}.$$

Add a potential energy $V(x,t)$ — kinetic plus potential equals total energy, the oldest story in classical mechanics — and you land on the *time-dependent Schrödinger equation*:
...
The equation is **linear**: if $\psi_1$ and $\psi_2$ are solutions, so is $\alpha\psi_1 + \beta\psi_2$. This is the superposition principle, and it is the algebraic origin of interference. Nonlinear wave equations do not give you interference.

The equation is **first-order in time**: fix $\psi$ at one instant and it is determined at all later times. The classical wave equation $\partial_t^2\psi = c^2\partial_x^2\psi$ is second-order in time and needs both $\psi$ and $\partial_t\psi$ at $t = 0$. The Schrödinger equation needs only $\psi$.

The equation carries **the factor of $i$**: drop it and the equation becomes $\partial_t\psi = (\hbar/2m)\partial_x^2\psi$ — the diffusion equation. Its solutions spread irreversibly; a localized initial condition smooths out and never recovers. The $i$ converts diffusion into wave propagation, and it is what keeps $\int|\psi|^2 dx$ constant in time. We'll prove that shortly.
...
None of these follows from the plausibility argument. They are properties the equation simply has. The argument motivates the *form* of the equation; experiment confirms that nature uses it.

To check at least for self-consistency, verify that $\psi(x,t) = A\,e^{i(kx-\omega t)}$ with $\omega = \hbar k^2/2m$ actually solves it. Left side: $i\hbar\,\partial_t\psi = i\hbar(-i\omega)\psi = \hbar\omega\psi$. Right side: $-(\hbar^2/2m)\partial_x^2\psi = (\hbar^2 k^2/2m)\psi$. Equal iff $\hbar\omega = \hbar^2 k^2/2m$, which is $E = p^2/2m$. The dispersion relation is the free-particle energy-momentum relation, as it had better be.

---

## The Born rule: what $|\psi|^2$ means

Schrödinger himself proposed that $|\psi|^2$ was the *charge density* of the electron — the electron as a smeared-out matter wave. That interpretation lasted weeks. Max Born proposed in 1926 [Born, *Z. Phys.* 37, 863–867](https://doi.org/10.1007/BF01397477) that

$$|\psi(x,t)|^2\,dx = \text{probability of finding the particle in } [x,\, x+dx] \text{ at time } t.$$

$\psi$ itself is not observable; $|\psi|^2$ is a probability density. Born got the Nobel Prize for this in 1954 — 28 years after the paper, which tells you how hard the probabilistic interpretation was to swallow through the twenties and thirties.

Schrödinger's matter-wave interpretation had two structural problems that Born's solved. First: $\psi$ is complex, and there is no classical notion of a complex matter density. You could try $\text{Re}(\psi)$ or $|\psi|$, but neither obeys a linear equation — you'd lose superposition. Second: for two particles, $\psi$ is a function of *six* coordinates, $\psi(x_1, y_1, z_1, x_2, y_2, z_2, t)$, and a function on six-dimensional configuration space cannot be a physical wave propagating in three-dimensional space. Born's interpretation handles both: probabilities are real and non-negative, and a probability distribution on configuration space is a perfectly sensible object regardless of dimension.

For the Born rule to be a consistent probability assignment, three conditions must hold. Non-negativity: $|\psi|^2 \geq 0$ everywhere — automatic, since it's a squared modulus. Normalization: $\int_{-\infty}^\infty |\psi(x,t)|^2\,dx = 1$ — the particle has to be *somewhere*. Conservation: if normalization holds at $t = 0$, it must hold at all later times. The third is not automatic. It is a theorem, and proving it requires the Hermiticity of $\hat{H}$.
...
What is the probability of finding the particle in the left quarter, $[0, L/4]$?
...
About 9%, not 25%. The ground-state density peaks in the center; the leftmost quarter is depleted. That number — $1/4 - 1/2\pi$ — comes entirely from the wave function's shape, not from any history of the particle. There is no trajectory. There is only the amplitude, squared.
...
Two misconceptions to dispose of now rather than later. First: "$\psi$ is the particle." No — $\psi$ is the probability amplitude. What registers in a detector is the particle; before detection, what exists is the amplitude. Second: "$|\psi|^2$ tells you what you don't yet know about the particle's actual position, which has some definite value you simply haven't measured." No again. Bell-inequality experiments — Aspect 1982, refined many times since, the 2022 Nobel Prize to Aspect, Clauser, and Zeilinger — rule out the local hidden-variable theories that would make that story work. The probability is irreducible. It is not a stand-in for incomplete classical knowledge.
...
From the Schrödinger equation, $\partial_t\psi = -(i/\hbar)\hat{H}\psi$. For real $V$, $\partial_t\psi^{*} = +(i/\hbar)(\hat{H}\psi)^{*}$. Substitute:
...
By Hermiticity of $\hat{H}$, $\langle\psi|\hat{H}\psi\rangle = \langle\hat{H}\psi|\psi\rangle$, so the bracket is zero. Normalization is preserved.

Notice what the proof consumed: exactly one fact, the Hermiticity of $\hat{H}$. This is the structural reason quantum mechanics postulates that observables are Hermitian operators. It is not a matter of mathematical taste. Without Hermiticity, the Born rule would fail to define a probability assignment that survives time evolution, and the whole apparatus would collapse.

The same argument, read locally rather than globally, produces the *continuity equation*. Define the probability density $\rho(x,t) = |\psi|^2$ and the probability current
...
This is the local statement of probability conservation. Probability does not vanish at one point and reappear elsewhere discontinuously — it flows. If probability is leaving a region, it crosses the boundary, and the rate of crossing is $j$.

![Diagram of a 1D region [a, b] with](../images/03-the-schrodinger-equation-fig-04.png)
*Figure 3.4 — Diagram of a 1D region [a, b] with*

To derive it: compute $\partial_t(\psi^{*}\psi)$, substitute the Schrödinger equation for $\partial_t\psi$ and $\partial_t\psi^{*}$, watch the potential terms cancel because $V$ is real, and recognize what's left as $\partial_x(\psi^{*}\partial_x\psi - \psi\partial_x\psi^{*})$ from the product rule. The whole thing is a half-page exercise; each step is just the Schrödinger equation applied twice, once to $\psi$ and once to $\psi^{*}$.

For a plane wave $\psi = Ae^{i(kx-\omega t)}$, the current works out to $j = (\hbar k/m)|A|^2 = v|A|^2$ — density times velocity, exactly the classical expression for a moving fluid. In the limit where the wave picture ought to look classical, the probability current looks like a particle current. Call it a sanity check on the internal consistency of the formalism.
...
Substitute. The left side gives $E\psi(x)e^{-iEt/\hbar}$; the right side gives $\hat{H}\psi(x)$ times $e^{-iEt/\hbar}$. The time-dependent phase cancels throughout, and what remains is:
...
Why "stationary"? Because $|\psi(x,t)|^2 = |\psi(x)|^2$ — the probability density is time-independent. Every measurement probability, for any time-independent observable, is constant. Looked at one piece at a time, the state does not evolve in any observable way.

And here is the misconception that ruins everything, in a very specific way: *stationary states are not static.* The *probability density* is constant. The *amplitude* $\psi(x,t) = \psi(x)\,e^{-iEt/\hbar}$ rotates in the complex plane at angular frequency $E/\hbar$. The magnitude at each $x$ is fixed; the phase advances at rate $E/\hbar$. For a single stationary state in isolation, that rotation has no observable consequence, because measurements depend only on $|\psi|^2$ and $|e^{-iEt/\hbar}| = 1$. You cannot see it. But put two stationary states together and you can see nothing else.
...
The cross term oscillates. It carries a factor $\cos((E_2 - E_1)t/\hbar)$ — the *Bohr frequency* $(E_2 - E_1)/\hbar$. The probability density rocks back and forth at this frequency, the expectation value of position oscillates, and when an atom in a higher state decays to a lower one it emits a photon of exactly the energy $E_2 - E_1$, producing a spectral line at frequency $(E_2 - E_1)/h$. Every spectral line in atomic physics — every line in the Balmer, Lyman, and Paschen series — exists because the phases of stationary states rotate, and their *relative* rotation at the Bohr frequency becomes observable the instant you superpose them.
...
That is the complete algorithm for time evolution under a time-independent Hamiltonian. Find the energy eigenstates. Expand the initial condition. Attach a phase to each term. Sum back up. Nearly every solvable problem in quantum mechanics — the infinite square well, the harmonic oscillator, hydrogen — runs this same algorithm with a different choice of $V(x)$, and therefore a different set of eigenfunctions $\psi_n$ and energies $E_n$.

The probability that an energy measurement yields $E_n$ is $|c_n|^2$, constant in time. The energy probabilities never change under Hamiltonian evolution, because each term's phase $e^{-iE_n t/\hbar}$ has modulus 1, so $|c_n e^{-iE_n t/\hbar}|^2 = |c_n|^2$. Only expectation values of *other* observables — position, momentum — can oscillate, and they do so because of the cross terms between energy eigenstates with different phases.

All of this requires a time-independent $V$. For time-dependent potentials the eigenstates themselves shift with time, and you need time-dependent perturbation theory. That is Unit 9.
...
This is not a failure of the formalism. Plane waves are generalized eigenfunctions in the sense flagged in Unit 2 — they live in the extended space beyond $L^2(\mathbb{R})$ and serve as calculational tools, just as the position eigenstates $|x\rangle$ do. Physical states are *wave packets*: superpositions of plane waves with a normalizable weight function. Write

$$\psi(x, 0) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^\infty \phi(k)\,e^{ikx}\,dk.$$

This is a Fourier expansion; $\phi(k)$ is the momentum-space wave function. If $\phi(k)$ is peaked near $k_0$ with spread $\Delta k$, then $\psi(x,0)$ is a localized wave packet with position spread of order $1/\Delta k$. That is the Heisenberg uncertainty relation: narrow in momentum, broad in position.
...
Because $\omega(k)$ is quadratic in $k$ — not linear — components at different momenta travel at different speeds. The wave packet *spreads*. For a Gaussian initial packet of width $\sigma_0$, the position-space width grows as $\sigma(t) = \sigma_0\sqrt{1 + (\hbar t/2m\sigma_0^2)^2}$. Slow at early times, linear in $t$ at late times. This spreading is one of the first genuinely quantum predictions you can compute and check — and for a particle it has no classical analog. A baseball thrown in a definite direction stays on its trajectory; a quantum particle disperses.
...
You now hold the dynamical law. Here is an honest reckoning of what it gives you and what it withholds.

It gives you a precise algorithm: write down $\hat{H}$, find its eigenstates and eigenvalues, expand your initial condition, attach phases, sum. Predictions about every measurement outcome, at any later time, with any observable, follow from this. The predictions are probabilities — not because of any ignorance on our part, but because the theory is genuinely probabilistic at the level of individual outcomes (Bell's theorem leaves no escape). The equation is linear, first-order in time, and Hermitian; normalization is preserved and the spectral lines come out right.

What it does not give you: an explanation of *why* this equation rather than its neighbors. The postulate status is honest, not evasive — Newton's second law has exactly the same status inside classical mechanics. The Schrödinger equation is tested by its consequences, and its consequences include the hydrogen spectrum, tunneling, the laser, the transistor, and the MRI machine. That is its justification.

What it also does not give you, and this is worth sitting with: it does not tell you what $\psi$ *is*. A real field in some physical sense? A representation of an observer's knowledge? A guiding wave for a hidden particle? The equation is silent. Every interpretation of quantum mechanics — many-worlds, Copenhagen, Bohmian mechanics, relational QM, the epistemic approaches — agrees on the predictions of the Schrödinger equation and disagrees on what the equation is describing. Unit 5 returns to this. For now: the formalism works regardless of which interpretation you favor, and the predictions are the same either way.
