...
When we first see the energy diagram for a particle trapped in a box, the levels appear as horizontal lines climbing up the page: $E_1, E_2, E_3, \ldots$ The lowest one sits above the bottom of the box, not at it. It is easy to copy down the formula, do the homework, and move on without pausing on a basic point: the bottom rung is not on the floor, and that fact deserves an explanation.

In classical mechanics, the bottom rung can be on the floor. A particle bouncing between two perfectly hard walls can, in principle, be brought to rest. Give it no kinetic energy, $E = 0$, and it simply sits there. Nothing in Newton's laws objects. But the quantum particle in the same box has a ground-state energy

$$E_1 = \frac{\pi^2 \hbar^2}{2mL^2}$$

that is strictly positive, that grows when we shrink the box, and that cannot be removed no matter how carefully we cool the system. We call this the zero-point energy, and it is not a quirk of boxes. The harmonic oscillator has it, a hydrogen atom has it, and liquid helium refuses to freeze at atmospheric pressure all the way down to absolute zero because the zero-point motion of the helium atoms is enough to keep them from locking into a lattice.

The reason is the uncertainty principle, stated quantitatively. Confine a particle to a box of width $L$. Its position uncertainty can be no larger than $L$, so $\sigma_x \lesssim L$. By the Robertson inequality (which we will derive in Chapter 5), the momentum uncertainty satisfies $\sigma_p \geq \hbar/(2\sigma_x)$, so $\sigma_p \gtrsim \hbar/(2L)$. The expected kinetic energy is therefore at least

$$\langle T \rangle = \frac{\langle p^2 \rangle}{2m} \geq \frac{\sigma_p^2}{2m} \gtrsim \frac{\hbar^2}{8mL^2}.$$

The exact box answer is $\pi^2\hbar^2/(2mL^2)$, while the uncertainty estimate gives $\hbar^2/(8mL^2)$. The two have the same form — both go as $\hbar^2/(mL^2)$ — and the factor of roughly 40 between them is geometry, not physics. The physics is that confinement forces the position uncertainty to be finite, finite position uncertainty forces nonzero momentum uncertainty, and nonzero momentum means nonzero kinetic energy. We cannot have a quantum particle that is both localized and at rest. The bottom rung is above the floor because there is no quantum state in which those two conditions hold at the same time.
...
Trap a particle between two perfectly hard walls at $x = 0$ and $x = L$. Inside, there are no forces; outside, the potential is infinite and $\psi = 0$ exactly. The time-independent Schrödinger equation inside is

$$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} = E\,\psi.$$

Write $k^2 = 2mE/\hbar^2$. The general solution is $\psi = A\sin(kx) + B\cos(kx)$. Applying the boundary conditions, $\psi(0) = 0$ kills the cosine, so $B = 0$, and then $\psi(L) = 0$ requires $kL = n\pi$ for integer $n$. Each integer gives an allowed wave vector and therefore an allowed energy. Normalizing $\int_0^L |\psi|^2\,dx = 1$ fixes $A = \sqrt{2/L}$:
...
The index starts at $n = 1$, not $n = 0$. Setting $n = 0$ gives $\psi \equiv 0$ everywhere, which is not a wave function but the absence of a particle. The ground state is $n = 1$, and its energy is exactly $E_1 = \pi^2\hbar^2/(2mL^2)$ — the positive floor we have been asking about.

There is a simple way to see the structure. The wave functions are standing waves in the box: $\sin(n\pi x/L)$ fits exactly $n$ half-wavelengths between the walls. More half-wavelengths means a shorter wavelength, which means a larger momentum, which means a larger kinetic energy. The energy spectrum $E_n \propto n^2$ is just the statement that squeezing more oscillations into a fixed interval costs energy quadratically. The $n = 1$ state has exactly one half-wavelength across the box, which is the least we can have while still satisfying both boundary conditions with a non-trivial function.
...
That is more than a third. The $n = 1$ wave function peaks in the middle (it has no nodes inside the well), so the center carries more probability than the edges. For $n = 2$, the wave function has a node at $x = L/2$, so the central third is depleted instead: $P_2 \approx 0.196$. At large $n$, the rapid oscillations of $\sin^2$ average to $1/2$, and $P_n \to 1/3$ — the classical answer, in which a uniformly bouncing particle spends equal time in equal lengths. This is one face of the correspondence principle: for large quantum numbers, the quantum distribution approaches the classical one.
...
Consider a particle in a parabolic potential $V(x) = \tfrac{1}{2}m\omega^2 x^2$. Classically, this is a mass on a spring. Quantum-mechanically, this single problem is the most-reused mechanism in all of physics: every smooth potential looks parabolic near a minimum, every mode of the electromagnetic field is one of these, every phonon in a crystal is one of these, and most of quantum field theory is built from them. We will work out the energy spectrum two ways — an algebraic route and an analytic route — because seeing two paths converge on the same answer is one of the best ways to gain confidence that we understand something.
...
*Fact 1.* Using $[\hat{x}, \hat{p}] = i\hbar$, we compute
...
So $\hat{a}_+|\psi\rangle$ is a new eigenstate with energy one rung higher, and similarly $\hat{a}_-|\psi\rangle$ has energy one rung lower. We can climb or descend the spectrum in steps of $\hbar\omega$. The reason the ladder has a bottom is that $\hat{H}$ is a sum of squared Hermitian operators and is therefore non-negative: $\langle\hat{H}\rangle \geq 0$ always. So there must be a state $|0\rangle$ that cannot be lowered any further — it satisfies $\hat{a}_-|0\rangle = 0$. Plugging that into the Hamiltonian:
...
The ground state here is labeled $n = 0$ (unlike the box, where it is $n = 1$) because the lowest-rung state is a genuine normalizable state rather than the trivial zero function.

A word on what the ladder operators actually are. In the quantization of the electromagnetic field, the same construction reappears: $\hat{a}^\dagger_k$ and $\hat{a}_k$ create and destroy photons of mode $k$. In condensed matter, they create and destroy phonons. The equation $\hat{H} = \hbar\omega(\hat{N} + 1/2)$ with $\hat{N} = \hat{a}_+\hat{a}_-$ is the prototype for second quantization. This is not an algebraic trick for avoiding a differential equation; it is the mechanism by which particles come into existence in quantum field theory. We are building that mechanism here, in its simplest form.
...
For large $|\xi|$, the solutions behave as $e^{\pm\xi^2/2}$, and only the decaying one is normalizable. We factor it out, writing $\psi = h(\xi)\,e^{-\xi^2/2}$. The remaining function $h$ satisfies the Hermite equation, and requiring the resulting power series to terminate (otherwise the wave function grows at infinity) forces $K = 2n + 1$, giving $E_n = \hbar\omega(n + 1/2)$. The terminating polynomials are the Hermite polynomials $H_n(\xi)$, and the ground state — with $H_0(\xi) = 1$ — is the Gaussian
...
We can derive this directly from $\hat{a}_-|0\rangle = 0$. In the position representation that condition reads

$$\frac{d\psi_0}{dx} = -\frac{m\omega}{\hbar}\,x\,\psi_0,$$

a first-order linear ODE whose solution is the same Gaussian. The algebraic route and the analytic route give the same function. That is not a coincidence — they are descriptions of the same physics — but seeing it explicitly removes any doubt about whether either derivation hides an error.

The width of the ground-state Gaussian is $x_0 = \sqrt{\hbar/(m\omega)}$. It depends only on $\hbar$ and the spring constant hidden in $\omega$. The ground state saturates the Robertson uncertainty bound: $\sigma_x\sigma_p = \hbar/2$ exactly, as you can verify by using the ladder operators to compute $\langle\hat{x}^2\rangle$ and $\langle\hat{p}^2\rangle$.

---

## The finite square well

Now pull the walls down from infinite to a finite depth $V_0$. Let $V(x) = -V_0$ for $|x| < a$ and zero outside. Two things happen that did not happen in the infinite well.

The first is that the spectrum splits into bound states (discrete, $E < 0$) and scattering states (continuous, $E > 0$). This is the finite well's version of the infinite well's isolated rungs — there are still discrete rungs, but now they sit in a finite potential, and above them lies a continuum.
...
The infinite square well, the harmonic oscillator, the finite square well, and the tunneling barrier are not four separate topics. They are four faces of the same thing: the interplay between confinement, uncertainty, and the exponential character of wave functions in classically forbidden regions.
...
Every finite barrier has a nonzero transmission probability, because the wave function decays exponentially but does not vanish inside classically forbidden regions. The rectangular barrier makes the calculation exact, and the WKB approximation extends it to realistic potentials. Alpha decay is the result, in one of the cleanest quantitative agreements between quantum mechanics and nuclear physics: a 1928 calculation, one page long, recovering measured half-lives across 24 orders of magnitude.

These are toy systems in the sense that they are exactly solvable and highly idealized. They are not toys in the sense that the physics they contain is serious. The harmonic oscillator alone underlies quantum optics, condensed matter physics, and quantum field theory. The tunneling barrier underlies the scanning tunneling microscope and the alpha-decay lifetime of every heavy nucleus ever measured. Solve these four systems well, and most of the rest of the course is variations on a theme.
