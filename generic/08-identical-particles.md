...
- The two electrons in helium are not really "electron 1 and electron 2," and treating them that way costs us real physics.
- We move through the exchange operator, the Slater determinant, the exchange correlation and how symmetry affects energy, and helium's singlet-triplet splitting.
...
Consider a question that sounds as if it ought to have a simple answer. You have two electrons in a helium atom. You measure the position of one and find it at $\mathbf{r}_1$. You measure the position of the other and find it at $\mathbf{r}_2$. The question is this: which electron did you measure first?

There is no answer. The difficulty is not that you forgot to label them, and not that your apparatus is imprecise. The two electrons are genuinely, irreducibly indistinguishable. No physical procedure — no color, no tag, no trajectory you could follow — separates "electron 1" from "electron 2." The Schrödinger equation for two electrons keeps the same structure when you swap their labels, and the wave function reflects that. A multi-particle wave function that treats the two electrons as distinct is, in a precise sense, telling us something false about the physics.

It would be natural to suppose this is a philosophical subtlety with no observable consequences. It is not. Whether the two-electron wave function is symmetric or antisymmetric under particle exchange determines which spin states helium's excited configurations can occupy. That in turn determines how often the two electrons sit close together, which determines the Coulomb energy by hundreds of millielectronvolts. The singlet-triplet splitting in helium — two families of states with measurably different energies, historically called parahelium and orthohelium — is an experimental fact that a theory built on labeled particles cannot even express, let alone predict.

And the reach goes well beyond helium. Without the antisymmetry constraint, every electron in every atom would pile into the $1s$ orbital. Shell structure would not exist. The periodic table would not exist. Chemistry would not exist. The indistinguishability of electrons is not a technicality. It is the foundation.

---

## The exchange operator

We define the *exchange operator* $\hat{P}_{12}$ on a two-particle wave function by

$$\hat{P}_{12}\,\psi(\mathbf{r}_1, \mathbf{r}_2) = \psi(\mathbf{r}_2, \mathbf{r}_1).$$

Two consequences follow at once. First, $\hat{P}_{12}^2 = \hat{1}$ — swap the labels twice and we return to where we started — so $\hat{P}_{12}$ has only the eigenvalues $\pm 1$. Second, if the particles are truly identical, every observable must be left unchanged by their exchange. In particular, the Hamiltonian has to commute with the exchange operator:

$$[\hat{H}, \hat{P}_{12}] = 0.$$

Operators that commute with $\hat{H}$ can be simultaneously diagonalized with it. So we are free to choose energy eigenstates that are also eigenstates of $\hat{P}_{12}$ — states that are either *symmetric* ($+1$ eigenvalue) or *antisymmetric* ($-1$ eigenvalue) under exchange.

Nature then makes one further choice that quantum mechanics itself does not derive: it selects one of the two options for each particle species, permanently. Electrons, protons, neutrons, and all other half-integer-spin particles are *fermions* — their multi-particle wave functions are antisymmetric under exchange. Photons, Higgs bosons, and all other integer-spin particles are *bosons* — their wave functions are symmetric. The link between spin and statistics is not a postulate of non-relativistic quantum mechanics but a theorem — Pauli's spin-statistics theorem, proved in 1940 [Pauli, *Physical Review* 58, 716–722](https://doi.org/10.1103/PhysRev.58.716) — that requires relativistic quantum field theory to derive. Within non-relativistic QM, the anti/symmetric assignment is simply postulated. It is worth knowing that the deeper theorem exists and that it draws on tools we have not yet built.
...
A note on what this postulate covers: it is not restricted to electrons that happen to be nearby. Two electrons at opposite ends of the galaxy are still identical, and in principle the wave function across all the electrons in the universe is antisymmetric under the exchange of any pair. In practice, exchange effects between electrons in distant atoms are negligible, because the overlap integral between their localized orbitals shrinks exponentially with distance. The antisymmetry holds globally; it only *matters* locally.

And one note for completeness: in two spatial dimensions, the argument breaks down. When two particles exchange positions in 2D, the path of exchange can wind around the relative-position singularity in topologically distinct ways, and the exchange phase is no longer restricted to $\pm 1$ — it can be any complex number $e^{i\theta}$. Particles with such intermediate statistics are *anyons*, and they have been observed in the fractional quantum Hall effect [Bartolomei et al. 2020, *Science* 368, 173–177](https://doi.org/10.1126/science.aaz5601). The $\pm 1$ dichotomy is a property of three spatial dimensions, not of quantum mechanics in general.

---

## The Slater determinant

For $N$ identical fermions, there is a clean way to write down the antisymmetric multi-particle wave function. We label the single-particle *spin-orbitals* (spatial wave function times spin state) as $\phi_1, \phi_2, \ldots, \phi_N$, and we label the particles as $1, 2, \ldots, N$. The antisymmetric state is
...
First, antisymmetry is built in. Swap two particle labels — that is, swap two columns of the determinant — and the determinant changes sign. That is exactly what determinants do when you exchange two columns. No extra work is needed; the algebra enforces the postulate.

Second, and this is the property that matters most for atomic physics: if two of the spin-orbitals are identical — two rows of the determinant are the same — the determinant is zero. The wave function vanishes identically. We cannot place two identical fermions in the same single-particle state, not because of a rule we once learned about quantum numbers, but because the antisymmetric wave function built from two identical orbitals is zero. This is Pauli exclusion expressed as a property of determinants.
...
Now we evaluate $\langle(x_1-x_2)^2\rangle$ for each case. Expanding the squared wave functions and integrating, the cross terms produce an *exchange integral* that adds to the fermion average separation and subtracts from the boson one. The result (Griffiths §5.1.2 carries this calculation) is

$$\langle(x_1-x_2)^2\rangle_\text{fermion} > \langle(x_1-x_2)^2\rangle_\text{distinguishable} > \langle(x_1-x_2)^2\rangle_\text{boson}.$$

Notice that we have not turned on any interaction term in the Hamiltonian. The particles do not interact. The difference in average separation comes entirely from the symmetry of the spatial wave function. Fermions sit statistically further apart than distinguishable particles would; bosons sit statistically closer. This is the *exchange correlation* — and that is the right name for it, not an "exchange force." There is no extra term in the Hamiltonian. There is only the Coulomb force and the antisymmetry constraint on the wave function. The antisymmetry dictates how often the two electrons sit close enough for the Coulomb repulsion to matter. That is the entire mechanism behind the helium spectrum.
...
The total wave function — spatial times spin — must be antisymmetric for electrons, since they are fermions. So:
...
The helium ground state has both electrons in the $1s$ orbital. The only spatial wave function available from two copies of $\psi_{1s}$ is the symmetric product $\psi_{1s}(\mathbf{r}_1)\psi_{1s}(\mathbf{r}_2)$ — automatically symmetric, because there is only one spatial orbital to work with. This forces the spin to be singlet. The ground state of helium is necessarily $S=0$ parahelium, with no other option.
...
We compute $\langle V_{ee}\rangle$ on the two spatial states. Expanding $|\psi_\pm|^2$, the diagonal terms give the *direct (Coulomb) integral*
...
Empirically, the orthohelium $1s2s$ state sits at $-59.2$ eV and the parahelium at $-58.4$ eV, a splitting of about 0.8 eV [NIST Atomic Spectra Database](https://www.nist.gov/pml/atomic-spectra-database). Orthohelium is lower, exactly as the calculation predicts.

![Helium energy level diagram ](../images/08-identical-particles-fig-03.png)
*Figure 8.3 — Helium energy level diagram *

It is worth being careful about what just happened. The spins did not interact directly — there is no $\hat{S}_1 \cdot \hat{S}_2$ term in the Hamiltonian at this order. What happened instead is that the spin state *forced the symmetry of the spatial wave function*, which forced how often the electrons sit near each other, which changed the Coulomb energy. The triplet has an antisymmetric spatial part: the electrons stay further apart, the Coulomb repulsion drops, and the energy comes out lower. The spin label is a marker for the spatial symmetry sector. The Coulomb force does the work; the antisymmetry channels it.
...
The physical content here is screening. An outer electron in a high-$\ell$ orbital is more effectively screened from the nucleus by the inner shells than an electron in a low-$\ell$ orbital at the same $n$, because low-$\ell$ orbitals penetrate the inner shells more deeply and so experience a larger effective nuclear charge. This is why $4s$ sits below $3d$ in energy for light atoms — the $4s$ electron, despite its larger principal quantum number, penetrates the core more effectively than the $3d$ electron and is therefore less screened.
...
The antisymmetric structure of fermion wave functions has consequences that reach far beyond atomic physics. When many fermions fill a band of single-particle states, they stack from the bottom up — each state takes at most one fermion — and at low temperature the distribution approaches a sharp step at the Fermi energy. The resulting *Fermi–Dirac distribution* is
...
The sign in the denominator is the whole story. At high temperature and low density, the two distributions converge to the classical Maxwell–Boltzmann form. At low temperature, they diverge: bosons condense, and fermions refuse to.
