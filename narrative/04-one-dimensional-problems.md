...
There is a question a student rarely thinks to raise the first time she meets the energy diagram for a particle trapped in a box. The levels are drawn as horizontal lines marching up the page: $E_1, E_2, E_3, \ldots$ The lowest of them sits above the bottom of the box rather than resting on it. The student copies the formula, finishes the homework, moves on. But the question is sitting right there, unasked: why isn't the bottom rung on the floor?

Classical mechanics permits it to be. A particle bouncing between two perfectly hard walls can, in principle, be brought to rest. Hand it no kinetic energy, $E = 0$, and there it sits. Newton's laws raise no objection. But the quantum particle in the very same box carries a ground-state energy

$$E_1 = \frac{\pi^2 \hbar^2}{2mL^2}$$

which is strictly positive, which climbs as you shrink the box, and which cannot be coaxed away no matter how patiently you cool the system. This is zero-point energy, and it is no peculiarity of boxes. The harmonic oscillator has it. A hydrogen atom has it. Liquid helium declines to freeze at atmospheric pressure all the way down to absolute zero because the zero-point motion of the helium atoms is by itself enough to keep them from locking into a lattice.

The reason is the uncertainty principle, stated with numbers. Confine a particle to a box of width $L$. Its position uncertainty can be no larger than $L$, so $\sigma_x \lesssim L$. By the Robertson inequality (which we derive in Chapter 5), the momentum uncertainty must obey $\sigma_p \geq \hbar/(2\sigma_x)$, so $\sigma_p \gtrsim \hbar/(2L)$. The expected kinetic energy is therefore at least

$$\langle T \rangle = \frac{\langle p^2 \rangle}{2m} \geq \frac{\sigma_p^2}{2m} \gtrsim \frac{\hbar^2}{8mL^2}.$$

The exact box answer is $\pi^2\hbar^2/(2mL^2)$; the uncertainty estimate gives $\hbar^2/(8mL^2)$. The forms match — both scale as $\hbar^2/(mL^2)$ — and the factor of roughly 40 separating them is geometry, not physics. The physics is that confinement forces position uncertainty to be finite, finite position uncertainty forces nonzero momentum uncertainty, and nonzero momentum means nonzero kinetic energy. You cannot have a quantum particle that is both localized and at rest. The bottom rung floats above the floor because there is no quantum state in which both conditions hold at once.

This chapter takes four specific potentials and works out, arithmetically, what that means.

---

## The infinite square well

Trap a particle between two perfectly hard walls at $x = 0$ and $x = L$. Inside, no forces act; outside, the potential is infinite and $\psi = 0$ exactly. The time-independent Schrödinger equation inside reads

$$-\frac{\hbar^2}{2m}\frac{d^2\psi}{dx^2} = E\,\psi.$$

Write $k^2 = 2mE/\hbar^2$. The general solution is $\psi = A\sin(kx) + B\cos(kx)$. Impose the boundary conditions: $\psi(0) = 0$ kills the cosine, $B = 0$. Then $\psi(L) = 0$ demands $kL = n\pi$ for integer $n$. Each integer hands you an allowed wave vector and therefore an allowed energy. Normalize $\int_0^L |\psi|^2\,dx = 1$ to fix $A = \sqrt{2/L}$:
...
The index starts at $n = 1$, not $n = 0$. Set $n = 0$ and you get $\psi \equiv 0$ everywhere — not a wave function at all, just the absence of a particle. The ground state is $n = 1$, and its energy is exactly $E_1 = \pi^2\hbar^2/(2mL^2)$ — the positive floor we were asking about.

The structure has a plain reading. The wave functions are standing waves in the box: $\sin(n\pi x/L)$ fits exactly $n$ half-wavelengths between the walls. More half-wavelengths means shorter wavelength, means larger momentum, means larger kinetic energy. The spectrum $E_n \propto n^2$ is simply the statement that cramming more oscillations into a fixed interval costs energy quadratically. The $n = 1$ state holds exactly one half-wavelength across the box, and that is the least you can fit while still satisfying both boundary conditions with a non-trivial function.
...
More than a third. The $n = 1$ wave function crests in the middle (no nodes inside the well), so the center hoards more probability than the edges. For $n = 2$, the wave function has a node at $x = L/2$, so the central third is starved instead: $P_2 \approx 0.196$. At large $n$, the rapid oscillations of $\sin^2$ average to $1/2$, and $P_n \to 1/3$ — the classical answer, in which a uniformly bouncing particle spends equal time in equal lengths. This is one face of the correspondence principle: for large quantum numbers, the quantum distribution drifts toward the classical one.
...
A particle in a parabolic potential $V(x) = \tfrac{1}{2}m\omega^2 x^2$. Classically, a mass on a spring. Quantum-mechanically, this single problem is the most heavily reused mechanism in all of physics: every smooth potential looks parabolic near a minimum, every mode of the electromagnetic field is one of these, every phonon in a crystal is one of these, and most of quantum field theory is assembled from them. I am going to show you two routes to the energy spectrum — an algebraic one and an analytic one — because watching two paths converge on a single answer is one of the surest ways to know you have understood something.
...
Here is the payoff. Suppose $|\psi\rangle$ is an eigenstate of $\hat{H}$ with energy $E$. Then

$$\hat{H}\hat{a}_+|\psi\rangle = (E + \hbar\omega)\,\hat{a}_+|\psi\rangle.$$

So $\hat{a}_+|\psi\rangle$ is a fresh eigenstate sitting one rung higher. Likewise $\hat{a}_-|\psi\rangle$ sits one rung lower. You can climb or descend the spectrum in steps of $\hbar\omega$. The ladder has a bottom because $\hat{H}$ is a sum of squared Hermitian operators and is therefore non-negative: $\langle\hat{H}\rangle \geq 0$ always. So there must be a state $|0\rangle$ that cannot be lowered any further — it satisfies $\hat{a}_-|0\rangle = 0$. Feed that into the Hamiltonian:
...
The ground state here carries the label $n = 0$ (unlike the box, where it is $n = 1$) because the lowest-rung state is a genuine normalizable state, not the trivial zero function.

A word on what the ladder operators truly are. In the quantization of the electromagnetic field, the identical construction returns: $\hat{a}^\dagger_k$ and $\hat{a}_k$ create and destroy photons of mode $k$. In condensed matter, they create and destroy phonons. The equation $\hat{H} = \hbar\omega(\hat{N} + 1/2)$ with $\hat{N} = \hat{a}_+\hat{a}_-$ is the prototype for second quantization. This is not an algebraic dodge to sidestep a differential equation; it is the very mechanism by which particles come into existence in quantum field theory. You are building that mechanism here, in its simplest incarnation.

**The analytic route.** The differential-equation approach makes the same point from another angle. In the dimensionless variable $\xi = \sqrt{m\omega/\hbar}\,x$, the Schrödinger equation becomes

$$\frac{d^2\psi}{d\xi^2} = (\xi^2 - K)\psi, \qquad K = \frac{2E}{\hbar\omega}.$$

For large $|\xi|$, solutions go as $e^{\pm\xi^2/2}$; only the decaying one is normalizable. Factor it out: $\psi = h(\xi)\,e^{-\xi^2/2}$. The remaining function $h$ obeys the Hermite equation, and demanding that the resulting power series terminate (otherwise the wave function blows up at infinity) forces $K = 2n + 1$, giving $E_n = \hbar\omega(n + 1/2)$. The terminating polynomials are the Hermite polynomials $H_n(\xi)$, and the ground state — with $H_0(\xi) = 1$ — is the Gaussian
...
You can also reach this straight from $\hat{a}_-|0\rangle = 0$. In the position representation that condition reads

$$\frac{d\psi_0}{dx} = -\frac{m\omega}{\hbar}\,x\,\psi_0,$$

a first-order linear ODE whose solution is the same Gaussian. The algebraic route and the analytic route deliver the same function. That is no accident — they describe the same physics — but seeing it laid out explicitly removes any lingering doubt that either derivation hides an error.

The width of the ground-state Gaussian is $x_0 = \sqrt{\hbar/(m\omega)}$. It depends on nothing but $\hbar$ and the spring constant buried inside $\omega$. The ground state saturates the Robertson uncertainty bound: $\sigma_x\sigma_p = \hbar/2$ exactly, as you can confirm by using the ladder operators to compute $\langle\hat{x}^2\rangle$ and $\langle\hat{p}^2\rangle$.
...
The first is that the spectrum splits into bound states (discrete, $E < 0$) and scattering states (continuous, $E > 0$). This is the finite well's counterpart to the infinite well's isolated rungs — there are still discrete rungs, but now they sit in a finite potential and above them stretches a continuum.

The second is the more striking. In the classically forbidden region outside the well, the wave function is not zero. Inside, $\psi$ oscillates. Outside, the Schrödinger equation gives $\psi'' = \kappa^2\psi$ with $\kappa = \sqrt{-2mE}/\hbar > 0$, and the normalizable solution is $\psi \propto e^{-\kappa|x|}$ — an exponentially decaying tail reaching into a region where a classical particle of the same energy could not exist. The particle leaks.
...
Matching $\psi$ and $d\psi/dx$ at $x = \pm a$ produces a transcendental equation. For even-parity bound states it takes the form

$$z\tan z = \sqrt{z_0^2 - z^2}, \qquad z = ka, \quad z_0^2 = \frac{2mV_0 a^2}{\hbar^2}.$$

The dimensionless depth $z_0$ fixes how many bound states fit. Very shallow wells support just one. Deeper wells support more, the count running roughly $z_0/(\pi/2)$ rounded up. One remarkable fact: in one dimension, a finite well always holds at least one bound state, no matter how shallow. This is special to one dimension and should not be carried over carelessly — a three-dimensional spherically symmetric well requires its depth to clear a critical threshold before it binds anything at all. A student who has only worked in one dimension risks absorbing the wrong lesson.

For an electron in a well of depth 50 eV and width 0.5 nm, the leakage depth is instructive. With $V_0 - E \approx 1$ eV into the barrier, $\kappa \approx 5.1\ \text{nm}^{-1}$, and at a distance $d = 1\ \text{nm}$ outside the wall the probability density is cut by $e^{-2\kappa d} \approx e^{-10.2} \approx 3.7 \times 10^{-5}$. Small. Not zero. The scanning tunneling microscope reads signals at exactly these suppression levels and converts them into atomic-resolution images of surfaces. The leakage is small enough to look negligible in a textbook problem and large enough to image atoms at Bell Labs.

---

## Tunneling and the WKB approximation

If the wave function leaks *into* a classically forbidden region, it can leak *through* one. A classical ball flung at a wall it cannot climb bounces back, every time, with certainty. A quantum particle aimed at the same barrier has a nonzero probability of materializing on the far side. This is tunneling, and the exponential decay of the wave function in the barrier is what governs how probable it is.
...
What makes this calculation worth dwelling on is the dynamic range. Alpha-decay half-lives across the periodic table span 24 orders of magnitude: from microseconds for Po-212 to $10^{17}$ years for Th-232. That entire span — 24 orders of magnitude — issues from modest changes in $Z$ and $E$ fed into an exponential. Po-212 has $E \approx 8.78$ MeV; Th-232 has $E \approx 4.01$ MeV. The energies differ by roughly a factor of two. The half-lives differ by a factor of $10^{24}$. The exponential does that. The Geiger-Nuttall law — the empirical observation, known since 1911, that a plot of $\log(\text{half-life})$ against $1/\sqrt{E}$ falls on a straight line — sat mysterious for nearly two decades. When Gamow's calculation arrived, it explained the straight line in one page of quantum mechanics.
...
The infinite square well, the harmonic oscillator, the finite square well, and the tunneling barrier are not four separate topics. They are four faces of one thing: the interplay among confinement, uncertainty, and the exponential character of wave functions in classically forbidden regions.
...
Every finite barrier has a nonzero transmission probability, because the wave function decays exponentially but does not vanish inside classically forbidden regions. The rectangular barrier makes the calculation exact. The WKB approximation carries it over to realistic potentials. Alpha decay is the result, in one of the cleanest quantitative agreements between quantum mechanics and nuclear physics: a 1928 calculation, one page long, recovering measured half-lives across 24 orders of magnitude.

These are toy systems in the sense that they are exactly solvable and heavily idealized. They are not toys in the sense that the physics inside them is serious. The harmonic oscillator alone underlies quantum optics, condensed matter physics, and quantum field theory. The tunneling barrier underlies the scanning tunneling microscope and the alpha-decay lifetime of every heavy nucleus ever measured. Solve these four systems well and most of the rest of the course is variations on a theme.
