...
- Look at the energy diagram for a particle in a box and ask the question nobody asks: why isn't the bottom rung on the floor? That single question opens the whole chapter.
- The chapter moves through The infinite square well, The harmonic oscillator — two routes to the same answer, The finite square well, Tunneling and the WKB approximation, and related ideas.
...
Look at the energy diagram for a particle trapped in a box. The levels climb up the page: $E_1, E_2, E_3, \ldots$ The lowest one sits *above* the bottom of the box, not at it. Most students copy down the formula, do the homework, and never ask the question staring back at them. Ask it now: why isn't the bottom rung on the floor? Before reading on, predict whether a trapped quantum particle could ever be brought completely to rest.

If you said yes, you are reasoning classically — and classically you'd be right. A particle bouncing between two perfectly hard walls can, in principle, be brought to rest. Give it no kinetic energy, $E = 0$, and it sits there. Nothing in Newton's laws objects. But here is where that intuition breaks: the quantum particle in the same box has a ground-state energy

$$E_1 = \frac{\pi^2 \hbar^2}{2mL^2}$$

which is strictly positive, which grows when you shrink the box, and which cannot be removed no matter how carefully you cool the system. This is zero-point energy, and it is not a quirk of boxes. The harmonic oscillator has it. A hydrogen atom has it. Liquid helium refuses to freeze at atmospheric pressure all the way to absolute zero because the zero-point motion of the helium atoms is enough to prevent them from locking into a lattice.

So *why* can't you remove it? The answer is the uncertainty principle, stated quantitatively — and you can almost derive the floor before being told it exists. Confine a particle to a box of width $L$. Its position uncertainty can be no larger than $L$, so $\sigma_x \lesssim L$. By the Robertson inequality (which we will derive in Chapter 5), the momentum uncertainty satisfies $\sigma_p \geq \hbar/(2\sigma_x)$, so $\sigma_p \gtrsim \hbar/(2L)$. The expected kinetic energy is therefore at least

$$\langle T \rangle = \frac{\langle p^2 \rangle}{2m} \geq \frac{\sigma_p^2}{2m} \gtrsim \frac{\hbar^2}{8mL^2}.$$

The exact box answer is $\pi^2\hbar^2/(2mL^2)$; the uncertainty estimate gives $\hbar^2/(8mL^2)$. Notice they have the same *form* — both go as $\hbar^2/(mL^2)$ — and the factor of roughly 40 between them is geometry, not physics. The physics is the chain you just walked: confinement forces position uncertainty to be finite, finite position uncertainty forces nonzero momentum uncertainty, and nonzero momentum means nonzero kinetic energy. You cannot have a quantum particle that is both localized and at rest. The bottom rung is above the floor because there is no quantum state where those two conditions hold simultaneously.

This chapter uses four specific potentials to work out what that means arithmetically.
...
Write $k^2 = 2mE/\hbar^2$. The general solution is $\psi = A\sin(kx) + B\cos(kx)$. Apply the boundary conditions: $\psi(0) = 0$ kills the cosine, $B = 0$. Then $\psi(L) = 0$ requires $kL = n\pi$ for integer $n$. Each integer gives an allowed wave vector and therefore an allowed energy. Normalize $\int_0^L |\psi|^2\,dx = 1$ to fix $A = \sqrt{2/L}$:
...
Why does the index start at $n = 1$ and not $n = 0$? Try $n = 0$ and see what it costs you: $\psi \equiv 0$ everywhere — not a wave function, just the absence of a particle. So the ground state is $n = 1$, and its energy is exactly $E_1 = \pi^2\hbar^2/(2mL^2)$ — the positive floor we were asking about.

There is a simple picture that makes the whole spectrum intuitive — and you can guess it. The wave functions are standing waves in the box: $\sin(n\pi x/L)$ fits exactly $n$ half-wavelengths between the walls. Now reason about energy: more half-wavelengths means shorter wavelength, means larger momentum, means larger kinetic energy. So the energy spectrum $E_n \propto n^2$ is just the statement that squeezing more oscillations into a fixed interval costs energy quadratically. The $n = 1$ state has exactly one half-wavelength across the box, and that is the least you can have while satisfying both boundary conditions with a non-trivial function.

**A probability calculation.** Here is a prediction to commit to first. For a particle in the $n = 1$ state, what is the probability of finding it in the *central third* of the box ($L/3 \leq x \leq 2L/3$) — exactly a third, more, or less?

$$P_1 = \frac{2}{L}\int_{L/3}^{2L/3}\sin^2\!\left(\frac{\pi x}{L}\right)dx = \frac{1}{3} + \frac{\sqrt{3}}{2\pi} \approx 0.609.$$

More than a third — why? The $n = 1$ wave function peaks in the middle (no nodes inside the well), so the center carries more probability than the edges. Now predict the $n = 2$ case before computing it: that wave function has a node at $x = L/2$, so the central third is *depleted* instead: $P_2 \approx 0.196$. And what happens as $n$ grows large? The rapid oscillations of $\sin^2$ average to $1/2$, and $P_n \to 1/3$ — the classical answer, in which a uniformly bouncing particle spends equal time in equal lengths. This is a face of the correspondence principle: for large quantum numbers, the quantum distribution approaches the classical one.
...
A particle in a parabolic potential $V(x) = \tfrac{1}{2}m\omega^2 x^2$. Classically, this is a mass on a spring — modest enough. So why call it the most important problem in physics? Here is the reason worth holding onto: every smooth potential looks parabolic near a minimum, every mode of the electromagnetic field is one of these, every phonon in a crystal is one of these, and most of quantum field theory is built from them. I am going to show you two routes to the energy spectrum — an algebraic one and an analytic one — and you should ask yourself, as we go, why bother with two. The answer: seeing two independent paths converge on the same answer is one of the best ways to be confident you understand something.
...
Now watch what that commutator does, and try to anticipate it. Suppose $|\psi\rangle$ is an eigenstate of $\hat{H}$ with energy $E$. Then

$$\hat{H}\hat{a}_+|\psi\rangle = (E + \hbar\omega)\,\hat{a}_+|\psi\rangle.$$

So $\hat{a}_+|\psi\rangle$ is a new eigenstate with energy one rung higher. Similarly $\hat{a}_-|\psi\rangle$ has energy one rung lower. You can climb or descend the spectrum in steps of $\hbar\omega$. But here is the question that decides whether the spectrum has a floor: can you keep lowering forever? No — because $\hat{H}$ is a sum of squared Hermitian operators and is therefore non-negative: $\langle\hat{H}\rangle \geq 0$ always. So there must be a state $|0\rangle$ that cannot be lowered further — it satisfies $\hat{a}_-|0\rangle = 0$. Plug that into the Hamiltonian:
...
Notice the ground state here is labeled $n = 0$ — unlike the box, where it was $n = 1$. Why the difference? Because the lowest-rung state here is a genuine normalizable state, not the trivial zero function.

Pause on what these ladder operators really are, because they are not a calculational trick. In the quantization of the electromagnetic field, the same construction reappears: $\hat{a}^\dagger_k$ and $\hat{a}_k$ create and destroy photons of mode $k$. In condensed matter, they create and destroy phonons. The equation $\hat{H} = \hbar\omega(\hat{N} + 1/2)$ with $\hat{N} = \hat{a}_+\hat{a}_-$ is the prototype for second quantization. This is the mechanism by which particles come into existence in quantum field theory. You are building that mechanism here, in its simplest form.

**The analytic route.** Would a completely different method give the same answer? Let's check by solving the differential equation directly. In dimensionless variable $\xi = \sqrt{m\omega/\hbar}\,x$, the Schrödinger equation becomes

$$\frac{d^2\psi}{d\xi^2} = (\xi^2 - K)\psi, \qquad K = \frac{2E}{\hbar\omega}.$$

For large $|\xi|$, solutions behave as $e^{\pm\xi^2/2}$; only the decaying one is normalizable. Factor it out: $\psi = h(\xi)\,e^{-\xi^2/2}$. The remaining function $h$ satisfies the Hermite equation, and requiring the resulting power series to terminate (otherwise the wave function grows at infinity) forces $K = 2n + 1$, giving $E_n = \hbar\omega(n + 1/2)$. Same spectrum. The terminating polynomials are the Hermite polynomials $H_n(\xi)$, and the ground state — with $H_0(\xi) = 1$ — is the Gaussian
...
Does the algebraic route reproduce this same Gaussian? It must, if the two routes are really describing one physics — derive it from $\hat{a}_-|0\rangle = 0$ and see. In position representation that condition reads

$$\frac{d\psi_0}{dx} = -\frac{m\omega}{\hbar}\,x\,\psi_0,$$

a first-order linear ODE whose solution is the same Gaussian. The algebraic route and the analytic route give the same function. That is not a coincidence — they are descriptions of the same physics — but seeing it explicitly removes any doubt about whether either derivation has a hidden error.

The width of the ground-state Gaussian is $x_0 = \sqrt{\hbar/(m\omega)}$. It depends only on $\hbar$ and the spring constant hidden in $\omega$. And here is a fact worth checking yourself: the ground state saturates the Robertson uncertainty bound: $\sigma_x\sigma_p = \hbar/2$ exactly, as you can verify using the ladder operators to compute $\langle\hat{x}^2\rangle$ and $\langle\hat{p}^2\rangle$.

---

## The finite square well

Now pull the walls down from infinite to a finite depth $V_0$. Let $V(x) = -V_0$ for $|x| < a$ and zero outside. Before reading on, predict what changes: with the walls no longer infinitely high, what can the particle now do that it couldn't before? Two things happen that did not happen in the infinite well.

The first is that the spectrum splits into bound states (discrete, $E < 0$) and scattering states (continuous, $E > 0$). This is the finite well's version of the infinite well's isolated rungs — there are still discrete rungs, but now they sit in a finite potential and above them is a continuum.

The second is more striking, and it is the one most students don't anticipate. Where is the particle allowed to be? In the classically forbidden region outside the well, the wave function is *not zero*. Inside, $\psi$ oscillates. Outside, the Schrödinger equation gives $\psi'' = \kappa^2\psi$ with $\kappa = \sqrt{-2mE}/\hbar > 0$, and the normalizable solution is $\psi \propto e^{-\kappa|x|}$ — an exponentially decaying tail that extends into the region where a classical particle with the same energy could not exist. The particle leaks.
...
The dimensionless depth $z_0$ determines how many bound states fit. Very shallow wells support only one. Deeper wells support more, with the count approximately $z_0/(\pi/2)$ rounded up. Now here is a fact worth pausing over and *not* over-generalizing: in one dimension, a finite well always has at least one bound state, no matter how shallow. Would you expect that to hold in three dimensions too? It does not — a three-dimensional spherically symmetric well requires the depth to exceed a critical threshold before it binds anything. A student who has only worked in one dimension is at risk of learning the wrong lesson.

How much does the leakage actually matter? For an electron in a well of depth 50 eV and width 0.5 nm, the leakage depth is instructive. With $V_0 - E \approx 1$ eV into the barrier, $\kappa \approx 5.1\ \text{nm}^{-1}$, and at distance $d = 1\ \text{nm}$ outside the wall the probability density is reduced by $e^{-2\kappa d} \approx e^{-10.2} \approx 3.7 \times 10^{-5}$. Small. Not zero. And that "not zero" is the whole point: the scanning tunneling microscope reads signals at exactly these suppression levels and turns them into atomic-resolution images of surfaces. The leakage is small enough that it looks negligible in a textbook problem and large enough to image atoms at Bell Labs.

---

## Tunneling and the WKB approximation

If the wave function leaks *into* a classically forbidden region, what is to stop it from leaking *through* one? Hold the contrast in mind: a classical ball thrown at a wall it cannot climb bounces back, every time, with certainty. Predict what the quantum particle does. A quantum particle aimed at the same barrier has a nonzero probability of appearing on the other side. This is tunneling, and the exponential decay of the wave function in the barrier is what controls how probable it is.
...
You have solved four potentials. Now ask: are these four separate topics, or four views of one thing? Look for what they share. The infinite square well, the harmonic oscillator, the finite square well, and the tunneling barrier are four faces of the same thing: the interplay between confinement, uncertainty, and the exponential character of wave functions in classically forbidden regions.

Every confined system has a positive energy floor, because confinement means finite position uncertainty means nonzero momentum means nonzero kinetic energy. That floor is the zero-point energy. The box makes it most visible. The oscillator makes it most reusable — every bound state in nature is approximately harmonic near its minimum, and the ladder operators that solve this one system are the prototype for the creation and annihilation operators of quantum field theory.

Every finite barrier has a nonzero transmission probability, because the wave function decays exponentially but does not vanish inside classically forbidden regions. The rectangular barrier makes the calculation exact. The WKB approximation extends it to realistic potentials. Alpha decay is the result, in one of the cleanest quantitative agreements between quantum mechanics and nuclear physics: a 1928 calculation, one page long, recovering measured half-lives across 24 orders of magnitude.

Are these merely toys? They are toys in the sense that they are exactly solvable and highly idealized. They are not toys in the sense that the physics they contain is serious. The harmonic oscillator alone underlies quantum optics, condensed matter physics, and quantum field theory. The tunneling barrier underlies the scanning tunneling microscope and the alpha-decay lifetime of every heavy nucleus ever measured. Solve these four systems well and most of the rest of the course is variations on a theme.
