...
- Five Tools for the Problems Schrödinger Couldn't Solve — and a question about which problems those actually are.
- Watch the turns: time-independent perturbation theory, degenerate perturbation theory, the variational principle, the WKB approximation.
...
Start with a question about your own experience of this course. Every problem you have solved so far — the infinite square well, the harmonic oscillator, the hydrogen atom — had an exact closed-form solution. So what would you conclude about quantum mechanics? The natural conclusion is that it is a subject where you write down the Schrödinger equation and solve it. Write that prediction down, because it is wrong, and seeing exactly how wrong it is reframes the rest of the field.

How short is the list of exactly-solvable quantum problems? Shorter than you would guess. Helium — three particles, one more Coulomb interaction than hydrogen — is already not on it. Real atoms beyond hydrogen, real molecules, interacting electrons in a solid, the vast majority of perturbations to any clean system: none of these admit closed-form eigenstates. Schrödinger published his equation in 1926 and solved the hydrogen atom that year. He did not solve helium in any closed form. Nobody has.

So if you cannot solve helium exactly, what did anyone do instead? What Egil Hylleraas published in 1929 was something different: a *variational* calculation. Pick a trial wave function with a few adjustable parameters. Compute the expectation value of the Hamiltonian as a function of those parameters. Minimize. With six parameters, Hylleraas reproduced the experimental ground-state energy of helium to four significant figures. Notice the trade he made: the wave function was approximate, but the number was right. This is the working mode of practical quantum mechanics — an approximate wave function that gives the right energy is good enough to build the next decade of physics on.
...
The setup: an exactly-solvable Hamiltonian $\hat{H}_0$ with known eigenstates $|n^{(0)}\rangle$ and energies $E_n^{(0)}$, and a small correction $\hat{H}'$ that you want to treat as a perturbation. The full Hamiltonian is $\hat{H} = \hat{H}_0 + \lambda\hat{H}'$, where $\lambda$ is a bookkeeping parameter you will set to 1 at the end. The whole method rests on one gamble: that the exact energies and states can be expanded in powers of $\lambda$:
...
Sandwich the perturbation between the unperturbed state and read off the shift. Simple — but here is a question the formula answers that you might not expect it to: does a perturbation always change the energy? Sometimes the answer is no, and the formula tells you immediately when.

For example: the first-order Stark effect on the hydrogen ground state. The perturbation is $\hat{H}' = e\hat{z}\mathcal{E}$ for an electric field $\mathcal{E}$ along $z$. Predict the sign and size of the shift before computing. The correction is

$$E_{1s}^{(1)} = e\mathcal{E}\int|\psi_{1s}|^2 z\,d^3r = 0,$$

zero — because $|\psi_{1s}|^2$ is spherically symmetric and $z$ is an odd function of position, so the integral of an odd times an even over all space vanishes. Hydrogen in the ground state has no first-order Stark effect. So where does the effect hide? It shows up at second order — the electric field mixes $|1s\rangle$ with excited states that *do* carry a nonzero $\langle z\rangle$, and second-order perturbation theory captures that mixing.

For the first-order **state** correction, project the same $\lambda^1$ equation onto $\langle m^{(0)}|$ for $m \neq n$:

$$|n^{(1)}\rangle = \sum_{m \neq n}\frac{\langle m^{(0)} | \hat{H}' | n^{(0)}\rangle}{E_n^{(0)} - E_m^{(0)}} |m^{(0)}\rangle.$$

Read the structure of that sum and ask where it could go wrong. The state mixes in contributions from every other unperturbed state, weighted by the off-diagonal matrix element of $\hat{H}'$ and divided by the energy gap. What happens as a gap shrinks? The smaller the gap, the larger the mixing — and the more suspicious you should be about convergence. Hold that suspicion; the next section is built on it.

The **second-order energy correction** is

$$E_n^{(2)} = \sum_{m \neq n}\frac{|\langle m^{(0)} | \hat{H}' | n^{(0)}\rangle|^2}{E_n^{(0)} - E_m^{(0)}}.$$

Now a prediction worth making: for the ground state, is this correction positive, negative, or could it go either way? Check the signs. For the ground state, every denominator $E_0^{(0)} - E_m^{(0)}$ is negative and every numerator is non-negative, so $E_0^{(2)} \leq 0$ always. The second-order correction to the ground-state energy is always non-positive. This is *level repulsion*: a perturbation pushes the ground state down and excited states up, more strongly the closer the levels are. The effect runs through band-structure physics, nuclear physics, and quantum chemistry wherever two levels come near each other and avoid crossing.
...
One thing to say explicitly about convergence, because the natural assumption is again wrong. Does the perturbation series converge? Often it does not — not in the usual sense. Freeman Dyson argued in 1952 that QED perturbation theory must be an asymptotic series with zero radius of convergence. The physical reason is worth following: if you flip the sign of the electric charge from $+e$ to $-e$, electrons would attract each other and the vacuum would be unstable. So the series cannot be analytic at the origin in the coupling constant, and the radius of convergence is zero. The series diverges if you sum it term by term — but the first few terms give excellent numerical results anyway, because the series is *asymptotically* accurate near the expansion point. Perturbation theory is useful precisely because the first few terms are accurate, not because the series converges.

---

## Degenerate perturbation theory

Return to the suspicion from the last section. When two or more unperturbed states share the same energy, what happens to the state-correction denominator $E_n^{(0)} - E_m^{(0)}$? It goes to zero and the formula diverges. So the method we just built breaks exactly when levels are degenerate. What is the fix? Conceptually simple: choose the *right* basis within the degenerate subspace before applying the perturbation.
...
What is the conceptual content? The unperturbed degeneracy did not pick out a preferred basis — any combination of the degenerate states was as good as any other. The perturbation breaks the tie and reveals which basis was the right one all along.

A clean example is the linear Stark effect on hydrogen $n=2$. The four degenerate states are $|2s\rangle, |2p_x\rangle, |2p_y\rangle, |2p_z\rangle$, all at the same energy. Before building the matrix, predict which of these the field can connect. The perturbation $\hat{H}' = e\hat{z}\mathcal{E}$ has matrix elements that vanish by parity except between $|2s\rangle$ and $|2p_z\rangle$ (the dipole operator $z$ connects states of opposite parity, and $|2s\rangle$ is even while $|2p_z\rangle$ is odd). The two-by-two block in the $\{|2s\rangle, |2p_z\rangle\}$ subspace is

$$W = \begin{pmatrix} 0 & -3e\mathcal{E} a_0 \\ -3e\mathcal{E} a_0 & 0 \end{pmatrix},$$

with eigenvalues $\pm 3e\mathcal{E} a_0$. The $|2p_x\rangle$ and $|2p_y\rangle$ states are untouched — no matrix elements connect them to anything. So the $n=2$ manifold of hydrogen splits *linearly* with the electric field: two outer states shifted by $\pm 3e\mathcal{E} a_0$, two inner states unshifted. This is the linear Stark effect. Why "linear," and why hydrogen specifically? It occurs because the accidental degeneracy of $|2s\rangle$ and $|2p_z\rangle$ in hydrogen allows the perturbation to mix them at first order in a way that produces a net dipole moment.
...
Perturbation theory needs a small parameter. What do you do when there isn't one — when the electron-electron repulsion in helium is not small at all? The variational method needs no small parameter. Here is its single inequality, and it is worth asking why it must be true before reading the proof. For any normalized trial state $|\psi_{\text{trial}}\rangle$,

$$\boxed{\langle\psi_{\text{trial}} | \hat{H} | \psi_{\text{trial}}\rangle \geq E_{\text{ground}}.}$$

Why can a random trial state never give you an energy below the true ground state? The proof is two lines. Expand $|\psi_{\text{trial}}\rangle = \sum_n c_n|n\rangle$ in the energy eigenbasis (which we do not know, but it exists). Then $\langle\hat{H}\rangle = \sum_n |c_n|^2 E_n \geq E_{\text{ground}} \sum_n |c_n|^2 = E_{\text{ground}}$. Equality holds only when all the weight is on the ground state. A mixture of energies always averages above the lowest one — you cannot beat the floor.

Three virtues follow, and each is worth naming. First, no expansion parameter is needed — you can apply it to helium without pretending the electron-electron repulsion is small. Second, the bound is one-sided: you can never *overestimate* the ground-state energy, so any trial state's number is an honest upper bound. Third — and this is the one that makes the method practical — the energy error is second order in the deviation of $|\psi_{\text{trial}}\rangle$ from the true ground state, so even a mediocre trial state gives a good energy estimate.

The helium calculation illustrates all three. What is a good guess for the trial state? Each electron sees the nucleus partly shielded by the other, so use a product of two hydrogen-like $1s$ orbitals but with an adjustable effective nuclear charge $Z^{*}$:
...
The experimental value is $-79.0$ eV. So how good is a one-parameter guess? Within 2% — and it comes with a physical story: each electron sees an effective nuclear charge of about 1.69 because the other electron screens 5/16 of the bare charge of 2.

![⟨H⟩(Z*) vs Z* for helium ](../images/09-approximation-methods-fig-03.png)
*Figure 9.3 — ⟨H⟩(Z*) vs Z* for helium *

One caution, because the method's success can mislead. Does a good variational *energy* mean you have a good *wave function* for everything? Not necessarily. The variational method gives you the ground-state energy reliably, not necessarily the ground-state wave function for other purposes. The energy converges much faster than the wave function, so variational energies are accurate to many decimal places in modern quantum chemistry while observables that depend sensitively on the wave function — like the charge density at the nucleus — require more care.

---

## The WKB approximation

The WKB approximation is the leading term in an expansion of the Schrödinger equation in powers of $\hbar$. What is the physical picture behind it? When the potential varies slowly compared to the local de Broglie wavelength, the wave function locally looks like a plane wave, and you can carry the local momentum along with you as the wave propagates.
...
In the classically forbidden region where $E < V(x)$, $p(x)$ becomes imaginary; the wave function decays exponentially instead of oscillating. When is the approximation trustworthy? When $|d\lambda/dx| \ll 1$ — when the de Broglie wavelength $\lambda = h/p$ changes slowly compared to itself.
...
Here is a fact to sit with before reading the explanation: alpha-decay half-lives across the periodic table span 24 orders of magnitude — from microseconds for some polonium isotopes to billions of years for thorium — for alpha-particle energies that differ by only a factor of two. How can a factor-of-two change in energy produce a $10^{24}$ change in lifetime? Look at where the energy enters: the integral inside the exponential is the action across the barrier, and small changes in barrier shape, fed through an exponential, produce enormous changes in transmission probability. That entire 24-order range comes from modest changes amplified by an exponential. George Gamow in 1928 derived the empirical Geiger-Nuttall law from exactly this WKB tunneling formula applied to the Coulomb barrier around a nucleus. One page of physics explained what had been mysterious since 1911.
...
Where does the $+1/2$ come from — why not just $n$, as Bohr's 1913 rule assumed? It is the Maslov index. It arises from matching the WKB solution to an Airy function at each turning point, where the potential is locally linear and the approximation must be patched. Two turning points, two matchings, a total phase contribution of $\pi/2$ — hence $(n+1/2)$ rather than the $n$ that Bohr's rule naively predicted. Applied to the harmonic oscillator, this recovers the exact spectrum $E_n = (n+1/2)\hbar\omega$.

---

## Time-dependent perturbation theory and Fermi's golden rule

Now the perturbation has a clock. $\hat{H}'(t)$ depends on time; you turn it on at $t=0$; the system starts in eigenstate $|i\rangle$ of the unperturbed Hamiltonian. The question we want answered: what is the probability of finding it in a different eigenstate $|f\rangle$ at time $t$?
...
where $\omega_{fn} = (E_f - E_n)/\hbar$. This is exact — an infinite set of coupled equations. Exact but unsolvable, so what do we approximate?
...
This is the central result. Let $\Omega = \omega_{fi} - \omega$ be the detuning from resonance. Before reading on, picture the shape of this as a function of $\Omega$ and ask what happens to it as time grows. The shape is a squared sinc function — peaked at $\Omega = 0$, width $\sim 1/t$, peak height $\sim t^2$. As $t \to \infty$, the peak becomes infinitely narrow and tall, approaching a delta function: in the distributional sense,
...
Look at the time dependence. The probability grows linearly in time — and what does a probability linear in time mean physically? A constant transition rate. The rate per unit time is the coefficient:
...
This is **Fermi's golden rule**. (A naming footnote worth its own irony: Dirac derived it in 1927. Fermi named it "Golden Rule No. 2" in his lecture notes. The misattribution stuck.)

It is worth pausing on what this formula contains, because each factor answers a different physical question. What sets the rate? Two things, multiplied. The matrix element $|\langle f|\hat{H}'|i\rangle|^2$ encodes the *coupling* between initial and final states — how strongly the perturbation connects them. The density of states $\rho(E_f)$ encodes how many final states are available at the resonance energy. Their product gives the rate — which means a large coupling into a sparse continuum can give the same rate as a weak coupling into a dense one.
...
But the rule does not apply everywhere, and its three conditions pull against each other — which is the subtle part. The *long-time* condition requires $t \gg 1/\omega_{fi}$ for the delta-function approximation to hold. The *first-order* condition requires $|c_f^{(1)}|^2 \ll 1$, so that replacing $c_n(t) \approx \delta_{ni}$ on the right-hand side is valid. And the *continuum* condition requires that final states be densely packed at $E_f = E_i + \hbar\omega$ — the formula does not apply to transitions between discrete bound states, where you get Rabi oscillations instead of a steady rate. Notice the tension between the first two: long time is needed for one but forbidden by the other. So the golden rule applies in a specific intermediate-time window, and recognizing that window is part of using it correctly.

---

## Five methods, five jobs

You now have five tools. The hard part is not deriving them — it is knowing which one a given problem is asking for. Let me say what each is actually for.
...
The methods above produce series or estimates. Here is a question that catches even careful students: when a method says "converges," does it mean the same thing each time? It does not, and knowing the difference matters as much as the formula.

Take perturbation theory in a coupling $\lambda$. Does its series converge? Often it produces a series that is asymptotic rather than convergent. The anharmonic oscillator with a $\lambda\hat{x}^4$ term has a perturbation series with zero radius of convergence (Bender and Wu showed this in 1969). So does QED. The series diverges if you sum all terms, but the first few terms give excellent numerical estimates — and those first few terms are what you actually compute. Asymptotic series are useful precisely because you stop before the terms start growing, which is where the estimate is best.

The variational principle is a different animal. It gives a *bound* — not an approximation that might be off in either direction. The energy estimate is guaranteed to be an overestimate of the true ground-state energy. That one-sidedness is a powerful property, and it is what made Hylleraas's 1929 calculation so convincing: his result was not just close to experiment, it was provably above the true ground-state energy.

And Fermi's golden rule is a third kind entirely. It is not a convergent series; it is a limit, valid in a specific time window. It fails qualitatively — not just quantitatively — when the conditions for the long-time, weak-coupling, continuum limit are not met.

So the real skill is diagnostic. Knowing which kind of approximation you are using — a convergent expansion, an asymptotic series, a variational bound, or a limiting formula — is as important as knowing the formula itself.
